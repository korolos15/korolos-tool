...
def search_for_updates():
    ...
    r = requests.get("korolos-tool/Discord-All-Tools-In-One/releases/latest")
    ...
    if THIS_VERSION not in result_string:
        ...
        soup = BeautifulSoup(requests.get("korolos-tool/Discord-All-Tools-In-One/releases").text, 'html.parser')
        for link in soup.find_all('a'):
            if "releases/download" in str(link):
                update_url = f"korolos-toolthub.com/{link.get('href')}"
        ...
        if choice.lower() == 'y' or choice.lower() == 'yes':
            ...
            if os.path.basename(sys.argv[0]).endswith("exe"):
                ...
                new_version_source = requests.get("korolos-tool/Discord-All-Tools-In-One/archive/refs/heads/master.zip")
